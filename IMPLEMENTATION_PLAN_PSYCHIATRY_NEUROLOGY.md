...
## Milestone 42: Automatische Diagnosevorschläge aus strukturierten Befunden

> **Priority:** P1 | **Estimated Effort:** Medium
> **Why:** Der Arzt dokumentiert Befunde strukturiert (AMDP, Neuro-Befund, Suizidalität), muss aber die ICD-10-Diagnosen manuell suchen und eingeben. Das System kann aus den Befundmustern automatisch passende Diagnosen vorschlagen. Der Arzt akzeptiert oder verwirft jeden Vorschlag mit einem Klick.

### 42.1 Architektur

Statischer Katalog (`DiagnosisSuggestionCatalog`) — kein neuer Service, keine DB, kein ML. Nimmt die bereits geladenen DTOs (AMDP-Findings, Neuro-Exam, Suizidalität) und gibt eine Liste von Vorschlägen zurück. Gleiche Pattern wie `DrugInteractionCatalog`.
...
public record DiagnosisSuggestion(
    ...
    string Rationale  // Welche Befunde den Vorschlag ausgelöst haben
);
```
...
|--------|--------|----------------|
| Depressive Episode leicht | F32.0 | AFF-5≥1 + 2-3 Stützkriterien (ANT-1, SOM-1, AFF-6, AUF-2, etc.) |
| Depressive Episode mittelgradig | F32.1 | AFF-5≥2 + 4+ Stützkriterien, Schwere ~2 |
| Depressive Episode schwer | F32.2 | AFF-5≥3 + 5+ Stützkriterien, Schwere 3 |
| Depressive Episode schwer + Psychose | F32.3 | F32.2-Kriterien + INH≥2 oder SIN≥2 |
| Manie | F30.1 | AFF-8≥2 + ANT-3 + FDS-7/8 |
| Bipolare Störung | F31.x | Manie + Depression gleichzeitig |
| Paranoide Schizophrenie | F20.0 | INH-5–14≥2 oder SIN-2≥2 oder ICH-3–6≥2 oder FDS-11≥2 |
| Wahnhafte Störung | F22.0 | INH≥2 ohne Halluzinationen/Ich-Störungen |
| Generalisierte Angst | F41.1 | AFF-7≥2 + AFF-11 + SOM-1 |
| Angst und Depression gemischt | F41.2 | AFF-7≥2 + AFF-5≥1 |
| Zwangsstörung | F42.0 | INH-1≥2 |
| Delir | F05.0 | BEW-1/2≥2 + ORI≥1 |
| Leichte kognitive Störung | F06.7 | AUF-3≥2 + AUF-4≥2 + ORI≥1 |
| Suizidalität | R45.8 | SUI-1–4≥1 oder RiskLevel≥Low |
...
| Polyneuropathie | G62.9 | Distale Sensibilitätsstörung + ASR bds. ≤1 |
...
### 42.5 UI — Sektion "Diagnosevorschläge"

Platzierung im EncounterFormular: zwischen "Strukturierte Befunde" und "Befund / Notizen". Erscheint nur bei vorhandenen Vorschlägen.

Jeder Vorschlag als Chip:
- Konfidenz-Indikator (grün/gelb/grau)
- ICD-10-Code + Beschreibung
- Rationale (welche Befunde ausgelöst haben)
- **+** grüner Button → übernimmt Code in ICD-10-Liste des Encounters
- **×** grauer Button → verwirft Vorschlag

### 42.6 Berechnung

- In `OnInitializedAsync` nach Laden der Findings-DTOs
- Synchroner Aufruf: `DiagnosisSuggestionCatalog.GetSuggestions(...)`
- Filtert bereits hinzugefügte und verworfene Codes
- Aktualisiert sich automatisch beim Neuladen der Seite (nach Rückkehr von AMDP/Neuro)

### 42.7 Fehlende ICD-10-Codes im Katalog ergänzen

```
F30.0  Hypomanie
F30.1  Manie ohne psychotische Symptome
F30.2  Manie mit psychotischen Symptomen
G81.9  Hemiparese, nicht näher bezeichnet
R29.1  Meningismus
